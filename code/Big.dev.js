...
//(function() { 
  ...
  function multiply(left, right) {
    var 
      wnorm = Math.max(left.whole.length, right.whole.length),
      lw = padLeft(left.whole, wnorm),
      rw = padLeft(right.whole, wnorm),

      fnorm = Math.max(left.fractional.length, right.fractional.length),
      lf = padRight(left.fractional, fnorm),
      rf = padRight(right.fractional, fnorm),

      polyProd = directPolyMult(
        strToPoly(lw + lf), 
        strToPoly(rw + rf)
      ),

      strProd = polyToStr(polyProd);

    return new Big(
      (left.sign == right.sign ? "+" : "-") +
        strProd.substring(0, strProd.length - (2 * fnorm)) + 
        "." + 
        strProd.substring(strProd.length - (2 * fnorm))
    );
  }
  
  function directPolyMult(left, right) {
    if (left.length != right.length) {
      throw "Bad direct mult args";
    }

    // An array of 2n-1 zeroes.
    var result = map(
      parseInt, 
      new Array(2 * left.length)
        .join(0)
        .split("")
    );

    for (var ridx = 0; ridx < right.length; ++ridx) {
      var digit = right[ridx];

      for (var lidx = 0; lidx < left.length; ++lidx) {
        result[ridx + lidx] += digit * left[lidx];
      }
    }
  ...
  function strToPoly(str) {
    return map(parseInt, str);
  }

  function polyToStr(poly, result) {
    ...
    if (last < 10) {
      ...
      if (len > 0) {
        return polyToStr(poly, ones + result);
      }
      else {
        return ones + result;
      }
  ...
  function divide(dividend, divisor) {
    var
      wnorm = Math.max(dividend.whole.length, divisor.whole.length),
      fnorm = Math.max(dividend.fractional.length, divisor.fractional.length),
      
      normend = triml(dividend.whole) + padRight(fnorm, dividend.fractional),
      normsor = triml(divisor.whole) + padRight(fnorm, divisor.fractional);
  }
  
  function recDiv(dividend, divisor, wholeMag, remainder, acc) {
    if (wholeMag == undefined) {
      wholeMag = triml(dividend.whole).length;
    }
